...
#define DEFINE_SYM_INST_UNI_OP_BV(name, bits)                                  \
  static inline void qce_sym_inst_bv##name##_i##bits(                          \
    ...
    qce_state_get_var(env, state, v, &expr_v);                                 \
                                                                               \
    QCEExpr expr_res;                                                          \
    qce_expr_bv##name##_i##bits(&state->solver_z3, &expr_v, &expr_res);        \
    qce_state_put_var(env, state, res, &expr_res);                             \
}

DEFINE_SYM_INST_UNI_OP_BV(not, 32)
DEFINE_SYM_INST_UNI_OP_BV(not, 64)

#define HANDLE_SYM_INST_UNI_OP_BV(key, name, bits)                             \
  case QCE_INST_##key##_I##bits: {                                             \
