...
void qce_on_tcg_tb_executed(TranslationBlock *tb, CPUState *cpu) {
  ...
  while (true) {
    ...
    case QCE_INST_GOTO_TB: {
       ...
       * been executed.
	   * Otherwise, just simply return and let QEMU dynamically decide which
	   * TB to be executed next.
	   */
      g_assert(!(((TranslationBlock *)next_tb)->cflags & CF_INVALID));
#ifdef QCE_DEBUG_IR
	  if (g_qce->trace_file != NULL) {
		fprintf(g_qce->trace_file, "<<<<\n");
	  }
    ...
    case QCE_INST_CALL_sgx: {
      ...
      HANDLE_SYM_INST_BIN_OP(ROTL, rotl, 32);
      HANDLE_SYM_INST_BIN_OP(ROTL, rotl, 64);

      HANDLE_SYM_INST_BIN_OP(ROTR, rotr, 32);
      HANDLE_SYM_INST_BIN_OP(ROTR, rotr, 64);
      ...
      HANDLE_SYM_INST_UNI_OP_BV(NOT, not, 32);
      HANDLE_SYM_INST_UNI_OP_BV(NOT, not, 64);
      ...
      HANDLE_SYM_INST_CALL_ld_i128;
      HANDLE_SYM_INST_CALL_st_i128;
