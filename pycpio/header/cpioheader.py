...
class CPIOHeader:
    ...
    def __init__(self, header_data=b"", overrides={}, *args, **kwargs):
        if header_data:
            ...
            raise NotImplementedError(
                "CPIOEntry must be initialized with header data or a name"
            )
    ...
    def from_bytes(self, data: bytes) -> None:
        ...
        if len(data) != 110:
            raise ValueError(
                "CPIO header must be 110 bytes, got length: %s" % len(data)
            )
        ...
        for attribute in self.structure:
            if attribute in overrides:
                if attribute == "mode":
                    ...
                    value = (int(self.mode, 16) & 0o7777000) | (
                        overrides[attribute] & 0o777
                    )
                else:
